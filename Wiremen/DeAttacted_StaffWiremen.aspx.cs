...
{
    public partial class DeAttacted_StaffWiremen : System.Web.UI.Page
    {
        CEI CEI = new CEI();
        ...
        public void GetContractorDetails(string Wiremen)
        {
            DataTable dt = new DataTable();
            dt = CEI.GetContractorDetails(Wiremen);
            if (dt.Rows.Count > 0)
            {
        ...
        protected void btnToDeattach_Click(object sender, EventArgs e)
        {
            if (Convert.ToString(Session["WiremanId"]) == hdnId.Value)
            {
                ...
                if (fileAttachment.HasFile && fileAttachment.PostedFile != null && fileAttachment.PostedFile.ContentLength <= maxFileSize)
                {
                    ...
                    string directoryPath = Server.MapPath("~/Attachment/Wiremen/DeAttached/" + txtReId.Text);
                    ...
                    string filePathInfo = "/Attachment/Wiremen/DeAttached/" + txtReId.Text + "/" + fileName;
