...
{
    public partial class AttachedRequestViewDetailsWiremen : System.Web.UI.Page
    {
        CEI CEI = new CEI();
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!Page.IsPostBack)
                {
                    ...
                    if (Convert.ToString(Session["WiremanId"]) != null && Convert.ToString(Session["WiremanId"]) != string.Empty && Convert.ToString(Session["Id"]) != null && Convert.ToString(Session["Id"]) != string.Empty)
                    {
                        hdnSupervisiorId.Value = Session["WiremanId"].ToString();
