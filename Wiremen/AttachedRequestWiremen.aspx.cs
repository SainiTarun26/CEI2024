...
{
    public partial class AttachedRequestWiremen : System.Web.UI.Page
    {
        CEI CEI = new CEI();
        ...
        protected void btnToDeattach_Click(object sender, EventArgs e)
        {
                ...
                if (Convert.ToString(Session["WiremanId"]) == hdnId.Value)
            {
                ...
                if (fileAttachment.HasFile && fileAttachment.PostedFile != null && fileAttachment.PostedFile.ContentLength <= maxFileSize)
                {
                    ...
                    {
                        string alertScript = "alert('Wiremen Id not find.');";
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "erroralert", alertScript, true);
                        return;

                    }
                    string directoryPath = Server.MapPath("~/Attachment/Wiremen/Attached/" + txtReId.Text);
                    ...
                    string filePathInfo = "/Attachment/Wiremen/Attached/" + txtReId.Text + "/" + fileName;
