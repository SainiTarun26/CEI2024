...
{
    public partial class Supervisor_Registration_Details : System.Web.UI.Page
    {
        CEI CEI = new CEI();
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    {
                        string SupervisorID = Convert.ToString(Session["SupervisorID"]);
                        GetUserDetails(SupervisorID);

                        hdnId.Value = Session["SupervisorID"].ToString();
                        //ApplicantDetail(SupervisorID);
                        //GetYearDropdown(YearDropdown);
                        //GetYearDropdown(DropDownList2);
                        //GetYearDropdown(DropDownList3);
                        //GetYearDropdown(DropDownList4);

                        GetEducationData(SupervisorID);
                        getcertificateofcompetencyDetails(SupervisorID);
                        PermanentEmployed(SupervisorID);
                        getExperienceDetail(SupervisorID);
                        RetiredEngineer(SupervisorID);
        ...
        private void RetiredEngineer(string supervisorID)
        {
            try
            {
                DataTable dt = new DataTable();
                dt = CEI.GetUserQualification(supervisorID);
        ...
        private void getExperienceDetail(string supervisorID)
        {
            try
            {
        ...
        private void PermanentEmployed(string supervisorID)
        {
            DataTable dt = new DataTable();
            dt = CEI.GetUserQualification(supervisorID);
        ...
        private void getcertificateofcompetencyDetails(string supervisorID)
        {
            try
            {
                DataTable dt = new DataTable();
        ...
        private void GetEducationData(string supervisorID)
        {
            try
            {
                DataTable dt = CEI.GetUserQualification(supervisorID);
        ...
        private void GetUserDetails(string supervisorID)
        {
            try
            {
                DataTable dt = new DataTable();
                dt = CEI.Getdetailsofuser(supervisorID);
