...
{
    public partial class SldApprovalRequest_AtOfficer : System.Web.UI.Page
    {
        ...
        private void GridBind()
        {
            if (Convert.ToString(Session["StaffID"]) != null && Convert.ToString(Session["StaffID"]) != string.Empty && Convert.ToString(Session["StaffID"]) == hdnStaffId.Value)
            {
                try
                {
                    ...
                    ds = CEI.ViewSldDocumentsFoApproval_AtOfficer(LoginId, txtSearch.Text);
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (Convert.ToString(Session["StaffID"]) != null && Convert.ToString(Session["StaffID"]) != string.Empty && Convert.ToString(Session["StaffID"]) == hdnStaffId.Value && ViewState["lblSldId"] != null && ViewState["lblSiteOwnerId"] != null)
            {
                if (Convert.ToString(ddlReview.SelectedItem) != "Select")
                {
                    ...
                    try
                    {
                        int x = CEI.SldApprovedByOfficer(SLDID, ddlReview.SelectedItem.ToString(), AdminId, filePathInfo, TxtRemarks.Text.Trim(), TxtRejectionReason.Text.Trim());
                    ...
                    try
                        ...
                        finally
                        {
                            ...
                            {
                                string script = $"alert('SLD Document {ddlReview.SelectedItem.ToString()} successfully.'); window.location='OfficerDashboard.aspx';";
