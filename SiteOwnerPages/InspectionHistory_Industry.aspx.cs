...
namespace CEIHaryana.SiteOwnerPages
{
    public partial class InspectionHistory_Industry : System.Web.UI.Page
    {
        ...
        public void BindGrid()
        {
            ...
            ds = CEI.SiteOwnerInspectionData_Industry(LoginID, Session["ReturnedTestReportId_Industry"].ToString());
            if (ds.Rows.Count > 0)
            {
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Select" || e.CommandName == "Print" || e.CommandName == "Print1")
            {
                ...
                {
                    Response.Redirect("/SiteOwnerPages/Inspection_Industry.aspx");
                }
                else if (e.CommandName == "Print")
                {
                    Response.Redirect("/SiteOwnerPages/InspectionRequestPrint_Industry.aspx");
                }
                else if (e.CommandName == "Print1")
                {
                    Response.Redirect("/Print_Forms/PrintCertificate1_Industry.aspx");
                }
            }
        }
