...
{
    public partial class Xen_To_XenLiftTransfer_ByAdmin : System.Web.UI.Page
    {
        ...
        public void BindStaff(string division)
         {
            ...
            ddlToAssign.Items.Add(new ListItem("XEN", "XEN"));
        }
        ...
        public void GetGridData()
        {
            try
            {
                ...
                DataSet ds = new DataSet();
                ds = CEI.InspectionLiftGridDataList_Admin(division, staff, district);
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            // Check for any selected inspection where the "AssignTo" column matches the selected assignee
            bool isDuplicateAssigneeFound = false;
            foreach (GridViewRow row in GridView1.Rows)
            {
                ...
                if (chk != null && chk.Checked)
                {
                    ...
                    if (lblAssignTo != null && lblAssignTo.Text == ddlNewAssignee.SelectedItem.Value)
                    {
                        isDuplicateAssigneeFound = true;
                        break;
                    }
                }
            }
            if (isDuplicateAssigneeFound)
            {
                Response.Write("<script>alert('Cannot transfer to the same assignee. Please select a different assignee.');</script>");
                return; 
            }
            ...
            try
            {
                ...
                //    {
                //        newReturnedTransferOrderId = CEI.Transfer_Order_Inspections_Attachments_XenToXen_ByAdmin(Convert.ToInt32(inspectionId), ddlNewAssignee.SelectedItem.Value, Session["AdminId"].ToString(), fixfileName);

                //        TransferAttachmentSaveMethod(fixfileName, newReturnedTransferOrderId);
                //        secondProcedureExecuted = true;
                //    }
                //    CEI.sp_Transfer_Inspections_XenToXenLift_ByAdmin_Method(Convert.ToInt32(inspectionId), ddlNewAssignee.SelectedItem.Value, Session["AdminId"].ToString(), newReturnedTransferOrderId);
                ...
                foreach (GridViewRow CurrentRow in GridView1.Rows)
                {
                    ...
                    {
                        Label lbllblInspectionIds = CurrentRow.FindControl("lblInspectionId") as Label;

                        if (lbllblInspectionIds != null)
                        {
                            if (!secondProcedureExecuted)
                            {
                                newReturnedTransferOrderId = CEI.Transfer_Order_Inspections_Attachments_XenToXen_ByAdmin(Convert.ToInt32(lbllblInspectionIds.Text), ddlNewAssignee.SelectedItem.Value, Session["AdminId"].ToString(), fixfileName);
                                TransferAttachmentSaveMethod(fixfileName, newReturnedTransferOrderId);
                                secondProcedureExecuted = true;
                            }
                            CEI.sp_Transfer_Inspections_XenToXenLift_ByAdmin_Method(Convert.ToInt32(lbllblInspectionIds.Text), ddlNewAssignee.SelectedItem.Value, Session["AdminId"].ToString(), newReturnedTransferOrderId);
            ...
            catch (Exception ex)
            {
                ...
                ScriptManager.RegisterStartupScript(this, this.GetType(), "showalert()", "alert('" + ex.Message.ToString() + "'); window.location.href = '/Admin/Xen_To_XenLiftTransfer_ByAdmin.aspx';", true);
                return;
            }
        }
        ...
        public void BindStaff_ToTransfer(string division)
        {
            DataSet ds = new DataSet();
            //ds = CEI.GetNewStaffByDistrictList(district, staffcurrentid);
            ds = CEI.GetNewXenTransferStaffByDivisionList(division);
