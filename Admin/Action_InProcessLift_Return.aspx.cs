...
namespace CEIHaryana.Admin
{
    ...
    public partial class Action_InProcessLift_Return : System.Web.UI.Page
    {
        ...
        {
            if (Convert.ToString(txtInspectionReportID.Text) != null && Convert.ToString(txtInspectionReportID.Text) != string.Empty && Convert.ToString(Session["AdminId"]) != null && Convert.ToString(Session["AdminId"]) != string.Empty)
            {
                                    ...
                                    if (ApprovedorReject == "Approved")
                                    {
                                        if (lblInspectionType.Text == "New")
                                        {
                                            foreach (GridViewRow row in Grid_MultipleInspectionTR.Rows)
                                            {
                                                ...
                                                string lblOwnerName = (row.FindControl("lblOwnerName") as Label)?.Text;
                                                ...
                                                CEI.InstallationApproval_Lift_New(ID, TestReportId, InstallationType, AdminId, lblInspectionType.Text, txtRegistrationNo.Text, TxtDivision.Text, lblMake, lblLiftSrNo, lblTypeOfLift,
                                                 lblTypeOfControl, lblCapacity, lblWeight, LblErectionDate, txtAddress.Text, txtDistrict.Text, DateTime.Parse(txtTranscationDate.Text), lblOwnerName, transaction);
                                        ...
                                        if (lblInspectionType.Text == "Periodic")
                                        {
                                            ...
                                            //CEI.InstallationApproval_Lift(ID, TestReportId, InstallationType, AdminId, lblInspectionType.Text, txtRegistrationNo.Text, DateTime.Parse(txtChallanDate.Text), TxtDivision.Text, lblMake, lblLiftSrNo, lblTypeOfLift,
                                            ...
                                            x += CEI.InstallationApproval_Lift(ID, TestReportId, InstallationType, AdminId, lblInspectionType.Text, txtRegistrationNo.Text, TxtDivision.Text, lblMake, lblLiftSrNo, lblTypeOfLift,
