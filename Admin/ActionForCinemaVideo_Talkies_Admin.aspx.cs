...
{
    public partial class ActionForCinemaVideo_Talkies_Admin : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    if (Convert.ToString(Session["AdminId"]) != null && Convert.ToString(Session["AdminId"]) != string.Empty && Convert.ToString(Session["InProcessInspectionId"]) != null && Convert.ToString(Session["InProcessInspectionId"]) != string.Empty)
                    {
                        hdnStaffId.Value = Session["AdminId"].ToString();
        ...
        private void GetData(string Id)
        {
            try
            {
                if (Convert.ToString(Session["AdminId"]) == hdnStaffId.Value && txtInspectionID.Text != null && txtInspectionID.Text != string.Empty)
                {
                    DataTable dt = CEI.GetSiteOwnerData_Cinema_Admin(Id);
        ...
        private void GetTestReportDataNew(string ID)
        {
            try
            {
                DataSet dsVC = CEI.GetDetailsToViewCinemaTestReport_Admin(ID);
                ...
                if (ClickCount < 1)
                {
                    ...
                    try
                    {
                        ...
                        if (ddlReview.SelectedValue != null && ddlReview.SelectedValue != string.Empty && ddlReview.SelectedValue != "0")
                        {
                            ...
                            // End ----------------------
                            int x = CEI.InspectionActionCinema_Admin(txtInspectionID.Text, hdnStaffId.Value, ApprovedorReject, txtRejectReason.Text, txtRemarks.Text, txtInspectionDate.Text, transaction);
                            if (x > 0)
                            {
                                checksuccessmessage = 1;
                                if (ApprovedorReject.Trim() == "Approved")
                                {
                                    int y = CEI.InsertApprovedCertificatedataCinema_Admin(txtInspectionID.Text, filePathInfo, transaction);
                                    if (y > 0)
                                    {
                                        transaction.Commit();
                                        string script = $"alert('Inspection  has been approved Successfully !!.'); window.location='/Admin/ActionInspectioHistrory.aspx';";
                                ...
                                {
                                    transaction.Commit();
                                    string script = $"alert('Inspection  has been Rejected.'); window.location='/Admin/ActionInspectioHistrory.aspx';";
                                    ScriptManager.RegisterStartupScript(this, this.GetType(), "SuccessScript", script, true);
