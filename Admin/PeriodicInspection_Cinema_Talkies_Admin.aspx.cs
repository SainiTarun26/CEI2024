...
{
    public partial class PeriodicInspection_Cinema_Talkies_Admin : System.Web.UI.Page
    {
        CEI CEI = new CEI();
        ...
        private void GetData(string Id)
        {
            try
            {
                if (Convert.ToString(Session["AdminId"]) == hdnStaffId.Value && txtInspectionID.Text != null && txtInspectionID.Text != string.Empty)
                {
                    ...
                    DataTable dt = CEI.GetPeriodicInspectionAdminOwnerData_Cinema(Id);
        ...
        private void GridToViewTRinMultipleCaseNew(string ID)
        {
            try
            {
                ...
                DataSet dsVC = CEI.GetPeriodicDetailsToViewCinemaTestReportAdmin(ID);
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
                        ...
                        if (RadioButtonList2.SelectedValue != "" && RadioButtonList2.SelectedValue != null)
                        {
                            ...
                            try
                            {
                                ...
                                if (RadioButtonList2.SelectedValue == "2")
                                {
                                    ...
                                    CEI.UpdateInspectionRejection_Periodic_Cinema_Admin(ID, StaffId, ddlRejectionReasonType.SelectedItem.ToString(), Reason);
                                ...
                                else
                                {
                                    if (InstallType == "Periodic")
                                    {
                                        if (RadioButtonList2.SelectedValue == "0")
                                        {
                                            ...
                                            CEI.InspectionAccepted_Periodic_Cinema_Admin(ID, StaffId);
                                            CCemail = Session["CCemail"].ToString();
                                            ToEmail = Session["ToEmail"].ToString();
                                            if (ToEmail.Trim() != "" && ToEmail != null)
                                            {
                                ...
                                if (checksuccessmessage == 1)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                ScriptManager.RegisterStartupScript(this, this.GetType(), "ErrorMessage", "alert('Inspection Returned to Site Owner'); window.location='IntimationHistoryForAdmin.aspx'", true);
            ...
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "ErrorMessage", "alert('You double click on Button.'); window.location='IntimationHistoryForAdmin.aspx'", true);
            }
        }
