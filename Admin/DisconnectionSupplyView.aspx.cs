...
{
    public partial class DisconnectionSupplyView : System.Web.UI.Page
    {
                ...
                if (!IsPostBack)
                {
                    ...
                    if (Convert.ToString(Session["AdminId"]) != null && Convert.ToString(Session["AdminId"]) != string.Empty)
                    {
                      if (Session["Dis_Id"] != null && Convert.ToString(Session["Dis_Id"]) != string.Empty)
                        {
                            hdnOwnerId.Value = Session["AdminId"].ToString();
        ...
        private void GetUtilityData(string DisId)
        {
            try
            {
                if (Convert.ToString(hdnOwnerId.Value) == Convert.ToString(Session["AdminId"]) && Convert.ToString(hdnDisId.Value) == Convert.ToString(Session["Dis_Id"]))
                {
        ...
        {
            if (Convert.ToString(hdnOwnerId.Value) == Convert.ToString(Session["AdminId"]))
            {
                Response.Redirect("/Admin/DisconnectionRequest.aspx", false);
            }
        }
