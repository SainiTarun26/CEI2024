...
{
    public partial class Print_Renewal_Of_Lift_PeriodicIndustryLift : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                try
                {
                    if (!IsPostBack)
                    {
                        if (Session["SiteOwnerId_PeriodicIndustryLift"] != null || Convert.ToString(Session["SiteOwnerId_PeriodicIndustryLift"]) != string.Empty)
                        {
                            GetData();
                        }
                        ...
                        {
                            Session["SiteOwnerId_PeriodicIndustryLift"] = "";
                            Session["AdminId"] = "";
                            Session["StaffID"] = "";
        ...
        public void GetData()
        {
            try
            {
                ...
                else if (Session["InspectionId_PeriodicIndustryLift"] != null)
                {
                    InspectionId = Session["InspectionId_PeriodicIndustryLift"].ToString();
                }
                else if (Session["InProcessInspectionId_PeriodicIndustryLift"] != null)
                {
                    InspectionId = Session["InProcessInspectionId_PeriodicIndustryLift"].ToString();
                }
                lblInspectionid.Text = InspectionId;
                ID = Session["LiftTestReportID_PeriodicIndustryLift"].ToString();
                DataTable dt = new DataTable();
                dt = CEI.GetLiftCertificateData_PeriodicIndustryLift(InspectionId, ID);
                if (dt.Rows.Count > 0)
                {
        ...
        protected void GridBind()
        {
            try
            {
                ...
                ds = CEI.getDataforLift_PeriodicIndustryLift(ID);
