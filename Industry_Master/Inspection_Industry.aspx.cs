    ...
    public partial class Inspection_Industry : System.Web.UI.Page
    {
        ...
        private void GetTestReportDataIfPeriodic()
        {
            try
            {
                ...
                ds = CEI.GetTestReportDataIfPeriodic_Industries(ID);
        ...
        public void GetDetailsWithId()
        {
            try
            {
                if (Session["InspectionId_Industry"] != null || !string.IsNullOrEmpty(Session["InspectionId_Industry"].ToString()))
                {
                    ID = Session["InspectionId_Industry"].ToString();
                    DataSet ds = new DataSet();
                    ds = CEI.InspectionData_Industries(ID);
        ...
        private void GridToViewCart()
        {
            try
            {
                string ID = Session["InspectionId_Industry"].ToString();
                DataSet dsVC = CEI.GetDetailsToViewCart_Industries(ID);
        ...
        protected void GridBind()
        {
            try
            {
                ID = Session["InspectionId_Industry"].ToString();
                DataSet ds = new DataSet();
                ds = CEI.ViewDocuments_Industries(ID);
        ...
        private void GetTestReportData()
        {
            try
            {
                ID = Session["InspectionId_Industry"].ToString();
                DataSet ds = new DataSet();
                ds = CEI.GetTestReport_Industries(ID);
                string TestRportId = string.Empty;
                if (ds != null && ds.Tables.Count > 0)
                {
        ...
        protected void buttonSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (IType == "New")
            {
                ds = CEI.ContractorRemarks_Industries(ID, TestReportId, txtOwnerRemarks.Text.Trim());
            }
            else if (IType == "Periodic")
            {
                ds = CEI.ContractorRemarksInPeriodic_Industries(ID, txtOwnerRemarks.Text.Trim());
            }
            //txtOwnerRemarks.Text = ds.Tables[0].Rows[0]["RemarkForContractor"].ToString();
        ...
        protected void btnResubmit_Click(object sender, EventArgs e)
        {
            ID = Session["InspectionId_Industry"].ToString();
            DataSet ds = new DataSet();
            ds = CEI.GetDetailsToViewCart_Industries(ID);
