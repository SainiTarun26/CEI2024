...
{
    public partial class Periodic_Renewal : System.Web.UI.Page
    {
        CEI CEI = new CEI();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!Page.IsPostBack)
                {
                    //Session["SiteOwnerId_Industry"] = "ABCDE5555O";
        ...
        private void GetDetails()
        {
            ...
            DataSet ds = new DataSet();
            ds = CEI.GetWorkIntimationDataAtSiteOwner_Industries(Id);
        ...
        protected void btnSubmitInstallation_Click(object sender, EventArgs e)
        {
            using (SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["DBConnection"].ConnectionString))
            {
                ...
                try
                {
                    ...
                    transaction.Commit();
                    Reset();
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "showalert", "alert('Details of Installation Submitted Successfully !!!'); window.location='Periodic_Installation_Rating.aspx';", true);
                }
                catch (Exception ex)
                {
