...
{
    public partial class EscalatorTestReportModal_IndustryLift : System.Web.UI.Page
    {
        CEI CEI = new CEI();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!Page.IsPostBack)
                {
                    ...
                    if (Convert.ToString(Session["EscalatorTestReportID_IndustryLift"]) != null && Convert.ToString(Session["EscalatorTestReportID_IndustryLift"]) != "")
                    {
                        TestReportId = Session["EscalatorTestReportID_IndustryLift"].ToString();
        ...
        private void GetDocument(string TestReportId)
        {
            ...
            ds = CEI.GetAttachments_IndustryLift(TestReportId.Trim());
            if (ds.Rows.Count > 0)
            {
        ...
        public void GetDetailswithId(string TestReportId)
        {
            try
            {
                ...
                ds = CEI.GetLiftTestReportModalData_IndustryLift("Escalator", TestReportId);
        ...
        {
            if (Session["PreviousPage_IndustryLift"] != null)
            {
                string previousPageUrl = Session["PreviousPage_IndustryLift"].ToString();
                Response.Redirect(previousPageUrl, false);
                Session["PreviousInspPage_IndustryLift"] = null;
            }
        }
    }
}