...
{
    public partial class EscalatorPeriodicTestReportModal_PeriodicIndustryLift : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!Page.IsPostBack)
                {
                    ...
                    else if ((Convert.ToString(Session["StaffID"]) != null || Convert.ToString(Session["StaffID"]) != string.Empty) && !string.IsNullOrEmpty(Convert.ToString(Session["RegistrationNo_PeriodicIndustryLift"])) && !string.IsNullOrEmpty(Convert.ToString(Session["TestReportID_PeriodicIndustryLift"])))
                    {
                        GetData();
