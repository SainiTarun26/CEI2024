...
{
    public partial class EscalatorPeriodicTestReportModal_IndustryLift : System.Web.UI.Page
    {
        CEI CEI = new CEI();
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!Page.IsPostBack)
                {
                    ...
                    else if ((Convert.ToString(Session["StaffID"]) != null || Convert.ToString(Session["StaffID"]) != string.Empty) && !string.IsNullOrEmpty(Convert.ToString(Session["RegistrationNo_IndustryLift"])) && !string.IsNullOrEmpty(Convert.ToString(Session["TestReportID_IndustryLift"])))
                    {
                        GetData();
