...
{
    public partial class LiftPeriodicRenewal_PeriodicIndustryLift : System.Web.UI.Page
    {
            ...
            try
            {
                if (!Page.IsPostBack)
                {
                    if (Convert.ToString(Session["SiteOwnerId_PeriodicIndustryLift"]) != null && Convert.ToString(Session["SiteOwnerId_PeriodicIndustryLift"]) != "")
                    {
                        ...
                        //{
                        //    Response.Redirect("InspectionHistory_Periodic_IndustryLift.aspx", false);
                        //    return;
                        //}

                        HdnUserID.Value = Convert.ToString(Session["SiteOwnerId_PeriodicIndustryLift"]);
                        //GetApplicantType();
                        if (Convert.ToString(Session["RegistrationNosessionPass_PeriodicIndustryLift"]) != null && Convert.ToString(Session["RegistrationNosessionPass_PeriodicIndustryLift"]) != "")
                        {
                            txtRegistrationNo.Text = Session["RegistrationNosessionPass_PeriodicIndustryLift"].ToString();
                            if (Session["InstallTypePass_PeriodicIndustryLift"] != null)
                            {
                                if (Session["InstallTypePass_PeriodicIndustryLift"].ToString() == "Lift")
                                {
        ...
        private void BindDistrict()
        {
            try
            {
                txtDistrict.Text = Session["district_PeriodicIndustryLift"].ToString();
                // Check if the district is "Gurugram"
                if (Session["district_PeriodicIndustryLift"].ToString().Equals("Gurugram", StringComparison.OrdinalIgnoreCase))
                {
        ...
        private void GetApplicantType(string UserID)
        {
            ...
            ds = CEI.GetApplicantTypeForLift_PeriodicIndustryLift(UserID);
        ...
        {
            DataSet ds = new DataSet();
            ds = CEI.GetRenewalLiftDataGridOfAllREcords_PeriodicIndustryLift(InstallationType, CreatedBy);
        ...
        private void GridDocument(string UserID)
        {
            ...
            DataTable ds = CEI.GetDocumentsForLiftRenewal_PeriodicIndustryLift(ApplicantType, InstallationTypeID, InspectionType, InstallationType);
        ...
        {
            string CreatedBy = HdnUserID.Value;
            int result = Convert.ToInt32(CEI.ToCheckeitherLiftOrEsclatorRegistered_PeriodicIndustryLift(txtRegistrationNo.Text.Trim(), CreatedBy));
            if (result == 1)
            {
                DataSet ds = CEI.GetRenewalLiftData_PeriodicIndustryLift(ddlInstallationType.SelectedItem.ToString(), txtRegistrationNo.Text.Trim(), CreatedBy);
                if (ds.Tables[0].Rows.Count > 0 && ds != null)
                {
                    ...
                    string areaValue = ds.Tables[0].Rows[0]["District"].ToString();
                    if (areaValue.StartsWith("Gurugram", StringComparison.OrdinalIgnoreCase))
                    {
                        txtDistrict.Text = "Gurugram";
                        divGurugramSelection.Visible = true;
                        ListItem selectedItem = ddlGurugram.Items.FindByValue(areaValue);
                        if (selectedItem != null)
                        {
                            ddlGurugram.ClearSelection();
                            selectedItem.Selected = true;
                            ddlGurugram.Enabled = false;
                        }
                    }
                    else
                    {
                        txtDistrict.Text = areaValue;
                        divGurugramSelection.Visible = false;
                        ddlGurugram.Enabled = true;
                    }
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Select")
            {
                ...
                DataSet ds = new DataSet();
                ds = CEI.GetRenewalLiftData_PeriodicIndustryLift(ddlInstallationType.SelectedItem.ToString(), RegistrationNo, CreatedBy);
                if (ds.Tables[0].Rows.Count > 0 && ds != null)
                {
                    txtMake.ReadOnly = true;
                    txtSerialNo.ReadOnly = true;
            ...
            try
            {
                if (Convert.ToString(Session["SiteOwnerId_PeriodicIndustryLift"]) != null && Convert.ToString(Session["SiteOwnerId_PeriodicIndustryLift"]) != "")
                {
                    ...
                    //if (Convert.ToString(HdnUserID.Value) != Convert.ToString(Session["SiteOwnerId"]) && Convert.ToString(HdnUserID.Value) != "")
                    {
                        ...
                        if (Convert.ToString(Session["SiteOwnerId_PeriodicIndustryLift"]) == Convert.ToString(HdnUserID.Value))
                        {
                            int result = Convert.ToInt32(CEI.ToCheckIfLiftOrEsclatorExistWithAnotherUser_PeriodicIndustryLift(txtRegistrationNo.Text.Trim(), Convert.ToString(HdnUserID.Value)));
                            ...
                            else
                            {
                                ...
                                string filePathInfo = "";
                                // Other fields
                                string districtValue = divGurugramSelection.Visible == false ? txtDistrict.Text : ddlGurugram.SelectedValue;
                                ...
                                if (Session["ReturnedValue_PeriodicIndustryLift"].ToString() != "1")
                                {
                                    ...
                                    string buisnessentity = string.Empty;
                                    string useremail = string.Empty;
                                    string mobile = string.Empty;
                                    if (Session["UserSessionData"] is Cei_IndustryServices_Redirection_IncomingJson_Model userSession)
                                    {
                                         username = userSession.uname;
                                         buisnessentity = userSession.businessentity;
                                         useremail = userSession.useremail;
                                         mobile = userSession.mobile;
                                    }
                                    else
                                    {
                                        string script = "alert('Session Expired'); window.location = 'https://staging.investharyana.in/#/';";
                                        ScriptManager.RegisterStartupScript(this, this.GetType(), "showalert", script, true);
                                    }

                                    TRID = CEI.InsertPeriodicLiftData_PeriodicIndustryLift(
                                        ...
                                        SiteOwnerID, username, buisnessentity, useremail, mobile, transaction);
                                }
                                else
                                {
                                    string TestReportId = Session["EscalatorTestReportID_PeriodicIndustryLift"].ToString();
                                    int InspectionId = int.Parse(Session["InspectionId_PeriodicIndustryLift"].ToString());
                                    DataTable dt = CEI.InsertReturnPeriodicLiftData_PeriodicIndustryLift(
                                ...
                                foreach (GridViewRow row in Grd_Document.Rows)
                                {
                                    ...
                                    if (fileUploadDoc1 != null && fileUploadDoc1.HasFile)
                                    {
                                        ...
                                        CEI.UploadDocumentforLiftPeriodic_PeriodicIndustryLift(TRID, txtRegistrationNo.Text, ddlInstallationType.SelectedItem.ToString(),
        ...
        private bool CheckInspectionStatus()
        {
            ...
            {
                panNumber = Session["SiteOwnerId_PeriodicIndustryLift"].ToString();
            }
            if (Session["district_PeriodicIndustryLift"] != null)
            {
                Districtlocalpr = Session["district_PeriodicIndustryLift"].ToString();
            }

            string connectionString = ConfigurationManager.ConnectionStrings["DBConnection"].ConnectionString;
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("sp_CheckAlreadyApplied_LiftInspection_PeriodicIndustryLift", conn))
                {
